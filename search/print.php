...
$objPHPExcel->getActiveSheet()->getDefaultColumnDimension()->setWidth(20);//設置單元格寬度
$objPHPExcel->getActiveSheet()->setTitle('論文詳細資料');//設置當前工作表的名稱

//註：單元格第一豎是以0開始的，第一行是以1開始的。
  $rowVal = array(0=>'著作名稱', 1=>'類型', 2=>'作者',3=>'發表時間',4=>'收錄期刊',5=>'期別/起訖頁');
...
for($i=2;$i<$total_records;$i++){
     ...
     $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $i, $row['magazine']);
     $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $i, $row['page']);
                                 }
